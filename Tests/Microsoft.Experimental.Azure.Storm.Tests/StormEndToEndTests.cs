...
namespace Microsoft.Experimental.Azure.Storm.Tests
{
	...
	public class StormEndToEndTests
	{
		...
		public void RunStormClusterTest()
		{
			...
			var zooKeeperTask = RunZooKeeper(zooKeeperDirectory);
			...
			var testJarTask = Task.Factory.StartNew(() => stormRunner.RunJar("SimpleTopology", testJar, runContinuous: false));
			var stormUITask = Task.Factory.StartNew(() => stormRunner.RunUI(false));
		...
		private static Task RunZooKeeper(string zooKeeperDirectory)
		{
